...
> 本表与 `docs/history/2026-08-13-exclusion-review/scope-exclusion-review-2026-08-13.md` §4 互补登记（**两表不重叠**：本表是代码层 24 项，scope-excl §4 是业务层 19 项）。本表 1-6 沿用 scope-excl §4 1-6；7-21 是 8/14 二次审计代码层新增（**2026-08-18 8/17 commit 5070ef9 落地后 #13 拆 3 行 / #19 拆 2 行，故代码层 21 → 24 项**）；#22 #23 跨表引用 scope-excl §4 18-19 真缺口。1-23 行时间窗全部基于 §1.3 三类分级（**已 approved 2026-08-14**）。#22 责任人 = SRE + 研发 / 时间窗 = 即时 ≤ 7 天；#23 责任人 = 测试治理 owner / 时间窗 = 周期 ≤ 30 天。来源列：行 1-6 沿用 scope-excl §4 1-6；行 7-21 为 8/14 二次审计代码层新增（#13 / #19 拆分后的额外 3 行来源标 8/17 commit 5070ef9）；行 22-23 跨表引用 scope-excl §4 18-19。
...
| 7 | 前端 Vue SFC（components / layouts / pages） | docs §4 8/14 二次审计新增 | vitest config 不 include `.vue`；PoC 演示由人工操作员，UI 行为不是 PoC 关键不变量 | Mavis | 客户端版本立项或增加 e2e 框架时纳入 | 战略 ≤ 90 天|
| 8 | `useDemoState` 客户端副作用（L17-26 Notification / L81-104 setInterval 轮询） | docs §4 8/14 二次审计新增 | 浏览器 only（`import.meta.client` / `document.*` / `window.setInterval`）；`use-demo-state.test.ts` 用 `import.meta.client=false` + stub 隔离；业务契约由 `state-endpoint` / `agent-tasks-endpoint` 间接锁 | Mavis | 增加 jsdom / happy-dom 集成测试或 Playwright 时纳入 | 战略 ≤ 90 天|
| 9 | `agent.ts` L300-311 `systemPrompt`（5 mode prompt 字符串模板） | docs §4 8/14 二次审计新增 | 纯字符串拼接，不是 logic；prompt 行为由 `agent-evaluation/core-regression.json` 100 用例锁定 | Mavis | 引入 prompt 单元化模板或 prompt version 概念时纳入 | 战略 ≤ 90 天|
| 10 | `agent.ts` L348-350 `response.usage` 三字段 / L347/L351 ternary | docs §4 8/14 二次审计新增 | v8 instrument 对行内对象属性 reporting 偏差（REAL-001 mock 显式传了 `prompt_tokens/completion_tokens/total_tokens` 已走过两条 arm） | Mavis | v8 升级或换 istanbul 时重新评估 | 战略 ≤ 90 天|
| 11 | `agent.ts` L523-524 `setTimeout(() => runTask(id, config), 40)` 调度 | docs §4 8/14 二次审计新增 | `setDeferAgentExecutionForTests(true)` 完全绕过；`runAgentTaskNow` 替代测试入口 | Mavis | 调度策略变更（如改 setImmediate / queue）时纳入 | 战略 ≤ 90 天|
| 12 | `parseJsonResponse` Array 分支（L216-218） | docs §4 8/14 二次审计新增 | `parse-json-response.test.ts` PJR-014 显式锁定为 dead branch（callModel 当前只对 string 调 parseJsonResponse） | Mavis | callModel 改为对数组也调 parseJsonResponse 时重新评估 | 战略 ≤ 90 天|
| 13 | `agent.ts` 防御性 `??` 兜底（L203/205/240-242/253-255/279-280/285-287/291/475 / L215 string-arm / L335/L347/L351） | docs §4 8/14 二次审计新增 | DB / JSON.parse / 字段填充 真防御性分支；happy path + 现有大量业务测试间接锁 | Mavis | DB schema 改 NOT NULL + 默认值 / OpenAI SDK 升级时重评 | 战略 ≤ 90 天|
| 13 | `agent.ts` L199 updateTask 容错（task 不存在时静默 return） | 8/17 commit 5070ef9 (test: 补真不变量) + 8/18 fresh coverage | 已被 8/17 commit 5070ef9 AGENT-LIFE-019 覆盖；fresh 8/18 实测 L199 已 100% 覆盖；保留登记是为了追踪 case 覆盖变化 | 测试治理 owner | updateTask 调用语义变更 / 取消静默 return 时重评 | 战略 ≤ 90 天|
| 13 | `agent.ts` L502 String(error) 错误序列化 + L322-323 LLM endpoint 配置守卫 | 8/17 commit 5070ef9 (test: 补真不变量) + 8/18 fresh coverage | 已被 8/17 commit 5070ef9 AGENT-LIFE-020 覆盖；L321-L323 endpoint 守卫由 `agent-callmodel-real.test.ts` REAL-001~005 直接锁(REAL-001~005 显式断言 throw 的是 L321 endpoint 守卫, 非间接)；保留登记是为了追踪 case 覆盖变化 | 测试治理 owner | error 抛出规范统一 / LLM endpoint 校验前置时重评 | 战略 ≤ 90 天|
| 14 | `action.post.ts` 防御性 `JSON.parse(x / '{}')` / `String(x / '')` 兜底（L21/22/31/49/50/60/79/93/116） | docs §4 8/14 二次审计新增 | 同上模式：业务契约由 happy path + `demo-actions-workflow.test.ts` (41) 间接锁 | Mavis | 同上 | 战略 ≤ 90 天|
| 15 | `state.ts` L5 `parseJson` falsy 兜底 / L98 `product ? : undefined` 防御性 guard | docs §4 8/14 二次审计新增 | `state.ts` parseJson 在所有 `*_json` 列上调用；fallback 路径仅在列 NULL 时触发 | Mavis | DB schema 改 NOT NULL 时重评 | 战略 ≤ 90 天|
| 16 | `identity.post.ts` L20/42 `JSON.parse(x / '{}')` 兜底 / L27 `email.split('@')[1] / ''`（zod `.email()` 已保证有 `@`） | docs §4 8/14 二次审计新增 | 防御性分支；L27 实际不可达 | Mavis | 去掉 zod email 校验或允许非标准邮箱时重评 | 战略 ≤ 90 天|
| 17 | `rematch.post.ts` L31 `JSON.parse(customer.facts_json  /  '{}')` 兜底 | docs §4 8/14 二次审计新增 | 防御性分支 | Mavis | 同上 | 战略 ≤ 90 天|
| 18 | `website.ts` L16-18 `JSON.parse(product.x_json  /  '[]')` 兜底 / L23 OR 第二段 `destination.includes(tail)` | docs §4 8/14 二次审计新增 | 防御性 nullish；L23 因 seed 路线 `normalized.includes(destination)` 总为 true 而被 OR 短路未求值（v8 artifact，行为正确） | Mavis | seed 路线改为无 `-` 分隔 / v8 升级时重评 | 战略 ≤ 90 天|
| 19 | `db.ts` L193 WAL fail catch / L206 `databasePath  /  default` / L246 `JSON.parse(pms_snapshot) catch` | docs §4 8/14 二次审计新增 | 环境/配置错误路径；需 host WAL reject / 空 config / 损坏 snapshot 才能触发；副作用大于价值 | Mavis | 增加故障注入框架（chaos / DB mock）时纳入 | 战略 ≤ 90 天|
| 19 | `db.ts` L295-298 resetDemoDatabase 事务完整性 ROLLBACK 真不变量 | 8/17 commit 5070ef9 (test: 补真不变量) + 8/18 fresh coverage | 已被 8/17 commit 5070ef9 DB-013 覆盖；fresh 8/18 实测 L295-298 已 100% 覆盖；事务完整性 + 一致性属 scope-policy.md 高风险条款；保留登记是为了追踪 case 覆盖变化 | 测试治理 owner | DB schema 变更时 / 引入新事务路径时重评 | 战略 ≤ 90 天|
| 20 | `customers.post.ts` L18 `Workbook has no worksheets` / L19 `workbook.Sheets[firstSheetName]` 访问 / L20 `worksheet unavailable` xlsx 错误路径 | docs §4 8/14 二次审计新增 | **dead-by-library**：`XLSX.write` 自带 `check_wb` 拒绝写 0-sheet workbook（throw "Workbook is empty"），`XLSX.read` 返回的 SheetNames/Sheets 一一对应；L18-20 在合法 xlsx 输入下不可达。catch 路径由 IMPORT-XLSX-012b（malformed zip）覆盖；8/18 fresh evidence 实测 L18-20 仍未覆盖，印证 dead-by-library 判定 | Mavis | xlsx 库大版本升级时重评 | 战略 ≤ 90 天|
