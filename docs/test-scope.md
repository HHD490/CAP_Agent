...
| 13 | `agent.ts` L502 String(error) 错误序列化 + L322-323 LLM endpoint 配置守卫 | 8/17 commit 5070ef9 (test: 补真不变量) + 8/18 fresh coverage | 已被 8/17 commit 5070ef9 AGENT-LIFE-020 覆盖；L322-323 由 `agent-callmodel-real.test.ts` REAL-001~005 间接锁；保留登记是为了追踪 case 覆盖变化 | 测试治理 owner | error 抛出规范统一 / LLM endpoint 校验前置时重评 | 战略 ≤ 90 天|
