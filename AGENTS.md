...
- **evidence-driven discovery: 5/5 subagent fail 是设计内 fail,不是 subagent 错误**(2026-08-19):scope_only 模式派 subagent 跑 5 个 OCU(Unicode/零宽/RTL/全角 bypass)真不变量 case,5/5 it fail 是**设计内的 fail** —— subagent 按任务 spec 写 expected,实际行为 ≠ expected,测试故意 fail 产出"漏洞证据"。正确流程:不撤销 subagent 改动 / 不让 CI 红 / 用 `it.skip`(XFAIL)中间态 + commit 保 evidence / 修完业务代码(`server/utils/contact.ts` 加 Unicode guard)后 unskip 改回 `it()`。实战:8/19 commit `e47572a`(XFAIL skip 5 OCU)→ `6257a99`(修 contact.ts + unskip)→ vitest 0 fail。反例:撤销 subagent 改动 / 让 CI 红 / 用 `it.todo` 假装没发现 / 改 expected 来"过"测试。配合:§6.4 "证据闭环" + "预算 ≠ 完成证据"。
- **subagent 透明披露事故 + owner 接受是健康协作模式**(2026-08-20):8/20 派 worker B 清临时文件,worker B transparent 披露 `git clean -fX -- <paths>` 误删 `coverage/` 目录(传显式路径时 `-X` 标志仍扩展到所有被 .gitignore 匹配的文件,实测如此)→ 立即 `npm run test:coverage` 重建(43 file / 640 test / 0 fail,与 baseline 一致)→ 在 final report 透明披露事故 + 重建证据 + owner 校验建议。Owner 跑 `git status --short`(0 untracked)+ `npm test`(640/0)独立验证后接受。启示:**subagent 透明披露事故 > 隐瞒失败**;**owner 接收 working tree 时必须独立跑 baseline 验证**,不能只信 subagent 自报。反例:worker 隐瞒误删 / 静默恢复不披露 / owner 不验证就接受。配合:§6.4 "subagent 纪律"(失败时派新 subagent,不接管编辑)。
